        ...
        enum msi_claw_mkeys_function mkeys)
{
	...
	const unsigned char buf[] = {
		FEATURE_REPORT_ID, 15, 0, 0, 60, MSI_CLAW_COMMAND_TYPE_SWITCH_MODE, (unsigned char)mode, (unsigned char)mkeys
	};
...
static int msi_claw_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if (drvdata == NULL) {
		hid_err(hdev, "Can't alloc MSI Claw descriptor\n");
		return -ENOMEM;
	}
